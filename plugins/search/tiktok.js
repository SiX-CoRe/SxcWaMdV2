...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) return m.reply(`⚠️ *Masukkan kata kunci pencarian TikTok!*\nContoh: ${usedPrefix + command} jedag jedug anime`);
  ...
  try {
    ...
    let caption = `🎬 *TIKTOK VIDEO SEARCH*\n\n`;
    ...
    caption += `\n✨ *SxcWaMd TikTok Downloader & Search*`;
    ...
    console.error(e);
    await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
    m.reply("❌ Gagal mencari video di TikTok: " + e.message);
  }
};

handler.help = ['tiktoksearch <query>', 'ttsearch <query>'];
handler.tags = ['search'];
