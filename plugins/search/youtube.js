...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  try {
    ...
    let output = `🎬 *HASIL PENCARIAN YOUTUBE*\n🔍 *Query:* ${text}\n\n`;
    ...
    console.error(e);
    await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
    m.reply("❌ Gagal mencari di YouTube: " + e.message);
  }
};

handler.help = ['youtube <query>', 'ytsearch <query>'];
handler.tags = ['search'];
handler.command = /^(youtube|ytsearch)$/i;
