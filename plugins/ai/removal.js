let handler = async (m, { conn, usedPrefix, command, text }) => {
  try {
    ...
    const response = await fetch(`${global.web}/api/ai/removal?apikey=${global.apikey.jereapi}&url=${encodeURIComponent(imgUrl)}`);
    ...
    if (json.status && typeof resultUrl === 'string' && resultUrl.startsWith('http')) {
      await conn.sendMessage(m.chat, {
        image: { url: resultUrl },
        caption: `✅ *Removal Background Success!*`
      }, { quoted: m }).catch(e => m.reply("Gagal mengirim media, ini linknya: " + resultUrl));
      await conn.sendMessage(m.chat, { react: { text: "✅", key: m.key } });
    } else {
      throw new Error(json.error || json.message || "Gagal menghapus background");
    }
  } catch (e) {
    console.error(e);
    await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
...
handler.help = ["removal <url/reply gambar>"];
handler.premium = true;
handler.command = ["removal", "removebg2"];
handler.tags = ["ai"];
handler.limit = 1;
