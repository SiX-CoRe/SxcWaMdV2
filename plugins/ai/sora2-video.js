let handler = async (m, { conn, usedPrefix, command, text }) => {
  try {
    if (!text) return m.reply(`⚠️ Masukkan prompt video Sora 2!\nContoh: ${usedPrefix + command} drone flyover ancient forest`);
    await conn.sendMessage(m.chat, { react: { text: "⏳", key: m.key } });
    m.reply("🎬 Sedang memproses Sora 2 AI Video... Mohon tunggu (estimasi 1 - 2 menit).");
    ...
    if (json.status && typeof videoUrl === 'string' && videoUrl.startsWith('http')) {
      await conn.sendMessage(m.chat, {
        video: { url: videoUrl },
        caption: `🎬 *Sora 2 AI Video Success!*\n\n📝 *Prompt:* ${text}`
...
handler.help = ['sora2-video <prompt>', 'sora2video <prompt>'];
handler.tags = ['ai'];
handler.premium = true;
handler.command = ["sora2-video", "sora2video", "sora2"];
handler.limit = 3;
