let handler = async (m, { conn, usedPrefix, command, text }) => {
  try {
    ...
    const response = await fetch(`${global.web}/api/ai/aiw3?apikey=${global.apikey.jereapi}&prompt=${encodeURIComponent(text)}`);
    ...
    if (json.status && json.result) {
      ...
      throw new Error(json.error || json.message || json.msg || "Gagal mendapatkan respons AIW3");
    }
...
handler.help = ["aiw3 <teks>"];
handler.premium = true;
handler.command = ["aiw3"];
handler.tags = ["ai"];
handler.limit = 1;
