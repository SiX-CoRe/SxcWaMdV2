let handler = async (m, { conn, usedPrefix, command, text }) => {
  try {
    ...
    const response = await fetch(`${global.web}/api/ai/iloveimg?apikey=${global.apikey.jereapi}&url=${encodeURIComponent(imgUrl)}`);
    const json = await response.json();
    
    let resultUrl = json.result?.image || json.result?.url || json.data?.image || json.data?.url || json.result || json.data;
    ...
    if (json.status && typeof resultUrl === 'string' && resultUrl.startsWith('http')) {
      await conn.sendMessage(m.chat, {
        image: { url: resultUrl },
        caption: `✅ *Compress Image Success!*`
...
handler.help = ["iloveimg <url/reply gambar>"];
handler.premium = true;
handler.command = ["iloveimg", "compressimage"];
handler.tags = ["ai"];
handler.limit = 1;
