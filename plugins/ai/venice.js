let handler = async (m, { conn, usedPrefix, command, text }) => {
  try {
    ...
    const response = await fetch(`${global.web}/api/ai/venice?apikey=${global.apikey.jereapi}&prompt=${encodeURIComponent(text)}&model=default`);
    ...
    if (json.status && json.result) {
      ...
      throw new Error(json.error || json.message || "Gagal mendapatkan respons dari Venice AI");
    }
