...
let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  try {
    ...
    const res = await fetch(`${global.web}/api/news/kompas?apikey=${apiKey}`);
    const json = await res.json();

    if (!json.status || !json.data || json.data.length === 0) {
      await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
      return m.reply("❌ Tidak ada berita Kompas yang ditemukan saat ini.");
    }

    const newsList = json.data.slice(0, 7);
    let text = `📰 *BERITA TERBARU KOMPAS.COM*\n\n`;
    ...
    console.error(e);
    await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
    m.reply("❌ Gagal memuat berita Kompas: " + e.message);
  }
};

handler.help = ['kompas', 'kompasnews'];
handler.tags = ['news'];
handler.command = /^(kompas|kompasnews)$/i;
