...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    try {
        ...
        let apiUrl = `${global.web}/api/tools/leopard-uploader?apikey=${apiKey}`;
        ...
        let caption = `📤 *UPLOAD BERHASIL (Leopard Hosting)*\n\n`;
        caption += `🔗 *URL:* ${fileUrl || 'Lihat respon API'}\n`;
        caption += `📦 *Ukuran:* ${sizeFormatted}\n`;
        caption += `📁 *Tipe:* ${mime}\n`;
        caption += `🌐 *Server:* Leopard Hosting\n\n`;
        ...
        console.error(e);
        await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
        m.reply(`❌ *Gagal Upload (Leopard Hosting)*\nError: ${e.message}`);
    } finally {
        setTimeout(() => {
...
handler.help = ["leoparduploader (reply media)"];
handler.command = ["leoparduploader","leopard"];
handler.tags = ['tools'];
