...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    try {
        ...
        let apiUrl = `${global.web}/api/tools/kappa-uploader?apikey=${apiKey}`;
        ...
        console.error(e);
        await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
        m.reply(`❌ *Gagal Upload (Kappa.lol)*\nError: ${e.message}`);
    } finally {
        setTimeout(() => {
...
handler.help = ["kappauploader (reply media)"];
handler.command = ["kappauploader","kappa"];
handler.tags = ['tools'];
