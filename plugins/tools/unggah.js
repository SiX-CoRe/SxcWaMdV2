...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    try {
        ...
        let apiUrl = `${global.web}/api/tools/unggah?apikey=${apiKey}`;
        ...
        let caption = `📤 *UPLOAD BERHASIL (Unggah.web.id)*\n\n`;
        caption += `🔗 *URL:* ${fileUrl || 'Lihat respon API'}\n`;
        caption += `📦 *Ukuran:* ${sizeFormatted}\n`;
        caption += `📁 *Tipe:* ${mime}\n`;
        caption += `🌐 *Server:* Unggah.web.id\n\n`;
        ...
        console.error(e);
        await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
        m.reply(`❌ *Gagal Upload (Unggah.web.id)*\nError: ${e.message}`);
    } finally {
        setTimeout(() => {
...
handler.help = ["unggah (reply media)"];
handler.command = ["unggah"];
handler.tags = ['tools'];
