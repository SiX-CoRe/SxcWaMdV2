...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    try {
        ...
        let apiUrl = `${global.web}/api/tools/uploadee?apikey=${apiKey}`;
        ...
        console.error(e);
        await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
        m.reply(`❌ *Gagal Upload (Upload.ee)*\nError: ${e.message}`);
    } finally {
        setTimeout(() => {
...
handler.help = ["uploadee (reply media)"];
handler.command = ["uploadee"];
handler.tags = ['tools'];
