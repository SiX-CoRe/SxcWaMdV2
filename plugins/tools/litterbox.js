...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    try {
        ...
        let apiUrl = `${global.web}/api/tools/litterbox?apikey=${apiKey}`;
        ...
        console.error(e);
        await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
        m.reply(`❌ *Gagal Upload (Litterbox)*\nError: ${e.message}`);
    } finally {
        setTimeout(() => {
...
handler.help = ["litterbox (reply media)"];
handler.command = ["litterbox"];
handler.tags = ['tools'];
