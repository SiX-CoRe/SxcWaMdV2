...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    try {
        ...
        let apiUrl = `${global.web}/api/tools/krakenfiles?apikey=${apiKey}`;
        ...
        let caption = `📤 *UPLOAD BERHASIL (Krakenfiles)*\n\n`;
        caption += `🔗 *URL:* ${fileUrl || 'Lihat respon API'}\n`;
        caption += `📦 *Ukuran:* ${sizeFormatted}\n`;
        caption += `📁 *Tipe:* ${mime}\n`;
        caption += `🌐 *Server:* Krakenfiles\n\n`;
        ...
        console.error(e);
        await conn.sendMessage(m.chat, { react: { text: "❌", key: m.key } });
        m.reply(`❌ *Gagal Upload (Krakenfiles)*\nError: ${e.message}`);
    } finally {
        setTimeout(() => {
...
handler.help = ["krakenfiles (reply media)"];
handler.command = ["krakenfiles","kraken"];
handler.tags = ['tools'];
