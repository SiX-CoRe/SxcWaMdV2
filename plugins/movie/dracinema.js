...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text && command !== 'dracinema') return m.reply(`Format salah!\n\nContoh: *${usedPrefix}${command} query*`)
    
    m.reply('Sedang memproses... Mohon tunggu.')
    try {
        let url = `${global.web}/api/movie/dracinema?apikey=${global.apikey.jereapi}&q=${encodeURIComponent(text)}`
...
handler.help = ['dracinema']
handler.tags = ['movie']
handler.command = /^dracinema$/i
