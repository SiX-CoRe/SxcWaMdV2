...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text && command !== 'moviebox_home') return m.reply(`Format salah!\n\nContoh: *${usedPrefix}${command} query*`)
    
    m.reply('Sedang memproses... Mohon tunggu.')
    try {
        let url = `${global.web}/api/movie/moviebox_home?apikey=${global.apikey.jereapi}`
...
handler.help = ['moviebox_home']
handler.tags = ['movie']
handler.command = /^movieboxhome$/i
