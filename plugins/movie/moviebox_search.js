...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text && command !== 'moviebox_search') return m.reply(`Format salah!\n\nContoh: *${usedPrefix}${command} query*`)
    
    m.reply('Sedang memproses... Mohon tunggu.')
    try {
        let [query, page, perPage, subjectType] = (text || '').split('|');
        let url = `${global.web}/api/movie/moviebox_search?apikey=${global.apikey.jereapi}&query=${encodeURIComponent(query || '')}&page=${encodeURIComponent(page || '')}&perPage=${encodeURIComponent(perPage || '')}&subjectType=${encodeURIComponent(subjectType || '')}`
...
handler.help = ['moviebox_search']
handler.tags = ['movie']
handler.command = /^movieboxsearch$/i
