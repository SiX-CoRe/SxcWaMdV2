...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text && command !== 'moviebox_detail') return m.reply(`Format salah!\n\nContoh: *${usedPrefix}${command} query*`)
    
    m.reply('Sedang memproses... Mohon tunggu.')
    try {
        let [path, subjectId] = (text || '').split('|');
        let url = `${global.web}/api/movie/moviebox_detail?apikey=${global.apikey.jereapi}&path=${encodeURIComponent(path || '')}&subjectId=${encodeURIComponent(subjectId || '')}`
...
handler.help = ['moviebox_detail']
handler.tags = ['movie']
handler.command = /^movieboxdetail$/i
