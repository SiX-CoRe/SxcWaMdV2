...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text && command !== 'hurawatch_movies') return m.reply(`Format salah!\n\nContoh: *${usedPrefix}${command} query*`)
    
    m.reply('Sedang memproses... Mohon tunggu.')
    try {
        let [filter, page] = (text || '').split('|');
        let url = `${global.web}/api/movie/hurawatch_movies?apikey=${global.apikey.jereapi}&filter=${encodeURIComponent(filter || '')}&page=${encodeURIComponent(page || '')}`
...
handler.help = ['hurawatch_movies']
handler.tags = ['movie']
handler.command = /^hurawatchmovies$/i
