...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text && command !== 'hurawatch_search') return m.reply(`Format salah!\n\nContoh: *${usedPrefix}${command} query*`)
    
    m.reply('Sedang memproses... Mohon tunggu.')
    try {
        let [query, page] = (text || '').split('|');
        let url = `${global.web}/api/movie/hurawatch_search?apikey=${global.apikey.jereapi}&query=${encodeURIComponent(query || '')}&page=${encodeURIComponent(page || '')}`
...
handler.help = ['hurawatch_search']
handler.tags = ['movie']
handler.command = /^hurawatchsearch$/i
