...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text && command !== 'moviebox_ranking') return m.reply(`Format salah!\n\nContoh: *${usedPrefix}${command} query*`)
    
    m.reply('Sedang memproses... Mohon tunggu.')
    try {
        let [id, page, perPage] = (text || '').split('|');
        let url = `${global.web}/api/movie/moviebox_ranking?apikey=${global.apikey.jereapi}&id=${encodeURIComponent(id || '')}&page=${encodeURIComponent(page || '')}&perPage=${encodeURIComponent(perPage || '')}`
...
handler.help = ['moviebox_ranking']
handler.tags = ['movie']
handler.command = /^movieboxranking$/i
